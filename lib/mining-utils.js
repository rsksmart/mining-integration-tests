...
async function promiseRequest(options, body) {
  return new Promise((resolve, reject) => {
    ...
    curl.setOpt(Curl.option.VERBOSE, options.verbose || false);
...
async function bitcoinGenerateBlocks(numberOfBlocks) {
  const addressResponse = await bitcoindPromiseRequest("getnewaddress", [], defaultContext);
  const address = JSON.parse(addressResponse).result;
  const generateResponse = await bitcoindPromiseRequest("generatetoaddress", [numberOfBlocks, address], defaultContext);
